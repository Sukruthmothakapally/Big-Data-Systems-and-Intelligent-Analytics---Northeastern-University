...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = '.\\pages\\servicekey.json'
...
st.markdown("<h2 style='font-weight: bold;'>Enter your OpenAI API key</h2>", unsafe_allow_html=True)
# User input for OpenAI API key as password
...
if openai_api_key:
    ...
    response = openai.Completion.create(
        ...
        prompt=f"Convert my short hand into a first-hand account of the meeting:\n\n{data}",
        temperature=0,
        max_tokens=100,
        top_p=1.0,
    ...
    # Display summary
    st.markdown(f"<h2 style='font-weight: bold;'>Summary of {company} for the year {year}</h2> {summary}", unsafe_allow_html=True)
...
if view_full_transcript == "Yes":
        ...
        # User input for Pinecone API key, environment, and index name
        pinecone_api_key = st.text_input("Enter your Pinecone API key", type="password")
        pinecone_environment = st.text_input("Enter your Pinecone environment",type="password")
        ...
        if pinecone_api_key and pinecone_environment and pinecone_index_name:
            ...
            for i in tqdm(range(0, len(chunks), batch_size)):
                ...
                try:
                    ...
                    st.write(f"Error while upserting batch {i} to {i_end}: {e}")
                
            # User input for query text
        query_text = st.text_input("Ask a question about the transcript")
        ...
        if query_text:
            ...
            # Display query results in Streamlit app
            ...
            #display langchain summary title
            st.markdown("<h2 style='font-weight: bold;'>LangChain Summary</h2>", unsafe_allow_html=True)
